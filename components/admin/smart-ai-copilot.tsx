...
export function SmartAICopilot() {
  ...
  const refreshAIRecommendations = async () => {
    if (customers.length > 0) {
      ...
      toast.success(aiEnhanced ? "AI recommendations refreshed" : "Using rule-based fallback")
    }
  }
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              ...
              <CardDescription>
                {aiEnhanced
                  ? "Suggestions generated by AI based on real customer data analysis"
                  : t("ai", "scDesc")}
              </CardDescription>
              {aiRecError && (
                <p className="text-xs text-orange-600 mt-1">AI unavailable: {aiRecError}</p>
              )}
            </div>
            <Button
              variant="outline"
            ...
            >
              ...
              {aiRecLoading ? "Analyzing..." : "Re-analyze with AI"}
            </Button>
          </div>
        </CardHeader>
        <CardContent>
          {recommendations.length === 0 ? (
            <p className="text-muted-foreground text-center py-4">
              {aiRecLoading ? "AI is analyzing customer data..." : t("ai", "scNoSuggestions")}
            </p>
          ) : (
